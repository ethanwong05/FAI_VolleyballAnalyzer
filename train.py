...
def train_model(rally_predictor, train_loader, val_loader, num_epochs=50, start_epoch=24):
    ...
    device, group_criterion, optimizer, person_criterion, rally_heuristic_criterion, rally_ranking_criterion = setup_model(
        rally_predictor, train_loader)
    ...
    load_checkpoint(best_val_group_acc, device, optimizer, rally_predictor, scheduler, start_epoch)

    print("Training...")
    group_acc, person_acc = training(accumulation_steps, best_val_group_acc, device, group_criterion, max_grad_norm,
                                     num_epochs, optimizer, persistent_dir, person_criterion, rally_heuristic_criterion,
                                     rally_predictor, rally_ranking_criterion, scaler, scheduler, start_epoch,
                                     train_loader, val_group_acc_history, val_loader)
    
    # Save final model
    save_model(group_acc, optimizer, persistent_dir, person_acc, rally_predictor, scheduler)


def save_model(group_acc, optimizer, persistent_dir, person_acc, rally_predictor, scheduler):
...
def training(accumulation_steps, best_val_group_acc, device, group_criterion, max_grad_norm, num_epochs, optimizer,
             persistent_dir, person_criterion, rally_heuristic_criterion, rally_predictor, rally_ranking_criterion,
             scaler, scheduler, start_epoch, train_loader, val_group_acc_history, val_loader):
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            with amp.autocast('cuda'):
                person_features, action_logits, group_logits, rally_probs, heuristic_probs = rally_predictor(images,
                                                                                                             bboxes)
        ...
        save_checkpoint(avg_group_loss, avg_person_loss, avg_rally_heuristic_loss, avg_rally_ranking_loss, epoch,
                        group_acc, optimizer, persistent_dir, person_acc, rally_predictor, scheduler)

        validation(avg_group_loss, avg_person_loss, avg_rally_heuristic_loss, avg_rally_ranking_loss,
                   best_val_group_acc, device, epoch, group_acc, group_criterion, left_team_activities, num_epochs,
                   optimizer, persistent_dir, person_acc, person_criterion, rally_heuristic_criterion, rally_predictor,
                   rally_ranking_criterion, right_team_activities, scheduler, val_group_acc_history, val_loader)
    return group_acc, person_acc


def validation(avg_group_loss, avg_person_loss, avg_rally_heuristic_loss, avg_rally_ranking_loss, best_val_group_acc,
               device, epoch, group_acc, group_criterion, left_team_activities, num_epochs, optimizer, persistent_dir,
               person_acc, person_criterion, rally_heuristic_criterion, rally_predictor, rally_ranking_criterion,
               right_team_activities, scheduler, val_group_acc_history, val_loader):
    ...
    if epoch % 5 == 0 or epoch == num_epochs:
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                with amp.autocast('cuda'):
                    person_features, action_logits, group_logits, rally_probs, heuristic_probs = rally_predictor(
                        images, bboxes)
        ...
        print(
            f"Moving Average Val Group Accuracy (last {len(val_group_acc_history)} validation points): {moving_avg_val_group_acc:.4f}")
...
def save_checkpoint(avg_group_loss, avg_person_loss, avg_rally_heuristic_loss, avg_rally_ranking_loss, epoch, group_acc,
                    optimizer, persistent_dir, person_acc, rally_predictor, scheduler):
    checkpoint = {
        'epoch': epoch,
        'rally_predictor_state_dict': rally_predictor.state_dict(),
        'optimizer_state_dict': optimizer.state_dict(),
...
def load_checkpoint(best_val_group_acc, device, optimizer, rally_predictor, scheduler, start_epoch):
...
def setup_model(rally_predictor, train_loader):
    ...
    return device, group_criterion, optimizer, person_criterion, rally_heuristic_criterion, rally_ranking_criterion
